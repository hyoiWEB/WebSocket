...
const posX = 0;
const posY = -1;
const posZ = -2.0; //モデルのサイズ

const scale = 1.0;
let renderer, scene, camera;
let loading; //THREEのレンダラの初期化
...
const loadModel = async () => {
  ...
  let result = await new Promise(resolve => {
    vrmLoader.load("assets/VRoid.vrm", vrm => {
      ...
      vrm.scene.rotation.set(0.0, Math.PI, 0.0); // VRMLoader doesn't support VRM Unlit extension yet so
      ...
      */
      ...
      scene.add(vrm.scene); //camera.lookAt(vrm.scene.position); 
...
const bindPage = async () => {
  ...
  */
  ...
  const resScene = initScene(); //レンダラ、シーンの初期化が済んでいるか
  ...
  const animate = async () => {
    ...
    poses.forEach(({
      ...
      if (joint["upperArmL"]) {
        if (joint["lowerArmL"]) {
          if (joint["upperArmR"]) {
            updateJoint(joint, "upperArmR", "upperArmL", "lowerArmL", dst["upperArmL"]); //adjJoint(0, -30, 0, dst["upperArmL"]); //ダミー
          }

          if (joint["handL"]) {
            updateJoint(joint, "upperArmL", "lowerArmL", "handL", dst["lowerArmL"]); //常に手のひらを向けるよう補正
            ...
            dst["handL"].rotation.setFromQuaternion(new THREE.Quaternion()); //adjJoint(0, 0, -20, dst["handL"]); //ダミー
          }
        }
      } //右腕
      ...
      if (joint["upperArmR"]) {
        if (joint["lowerArmR"]) {
          if (joint["upperArmL"]) {
            updateJoint(joint, "upperArmL", "upperArmR", "lowerArmR", dst["upperArmR"]); //adjJoint(0, 30, 0, dst["upperArmR"]); //ダミー
          }

          if (joint["handR"]) {
            updateJoint(joint, "upperArmR", "lowerArmR", "handR", dst["lowerArmR"]); //常に手のひらを向けるよう補正
            ...
            dst["handR"].rotation.setFromQuaternion(new THREE.Quaternion()); //adjJoint(0, 0, 20, dst["handR"]); //ダミー
          }
        }
      } //胸
      ...
      */
      ...
      if (joint["upperLegL"] && joint["upperLegR"]) {
        ...
        dst["hips"].position.x = x * adjX; //adjJoint(-20, 0, 0, dst["spine"]); //ダミー
      } //左脚
      ...
      if (joint["upperLegL"]) {
        if (joint["lowerLegL"]) {
          ...
          if (joint["footL"]) {
            updateJoint(joint, "upperLegL", "lowerLegL", "footL", dst["lowerLegL"], -20, 20); //基準点が無いので左足首の下垂直方向に仮のジョイントを作る
      ...
      if (joint["upperLegR"]) {
        if (joint["lowerLegR"]) {
          ...
          if (joint["footR"]) {
            updateJoint(joint, "upperLegR", "lowerLegR", "footR", dst["lowerLegR"], -20, 20); //基準点が無いので右足首の下垂直方向に仮のジョイントを作る
